...
pub struct ExpireDuelChallenge<'info> {
    ...
    /// CHECK: opponent wallet for PDA derivations
...
pub fn handler(ctx: Context<ExpireDuelChallenge>, _challenge_seed: u64) -> Result<()> {
    ...
    require!(
        ctx.accounts.authority.key() == duel_challenge.challenger
            || ctx.accounts.authority.key() == duel_challenge.opponent,
        ChainDepthError::Unauthorized
    );
    require!(
        Clock::get()?.slot > duel_challenge.expires_at_slot,
        ChainDepthError::DuelChallengeNotExpired
    );
    ...
    duel_challenge.status = DuelChallenge::STATUS_EXPIRED;
    duel_challenge.settled_slot = Clock::get()?.slot;

    emit!(DuelChallengeExpired {
